      ...
      <header className="bg-navy text-white sticky top-0 z-20 border-b border-white/10">
        <div className="max-w-3xl mx-auto px-4 py-3 md:py-4 flex flex-col gap-4">
          ...
          <div className="flex justify-between items-start gap-4">
            <div>
              <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-white drop-shadow-sm mb-1 sm:mb-2">
                {settings?.shopName || "Supreme Sanitary"}
              </h1>
              <p className="text-white/60 text-xs sm:text-sm leading-snug max-w-[280px] sm:max-w-xl">
                Sanitary Pipes · Plumbing Pipes · Fittings · Chamber Covers · Garden & Suction Hose · Water Tanks & Bathroom Accessories
              </p>
            </div>
            <a
              href="/shop"
              className="text-sm text-white/50 border border-white/20 hover:text-white hover:border-white/50 transition rounded-full px-4 py-2 whitespace-nowrap shrink-0 mt-1"
            >
              Shop login
            </a>
          </div>
          ...
          {/* Contact Info Card */}
          <div className="bg-white/5 border border-white/5 rounded-xl p-4 sm:p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
            
            {/* Address */}
            <div className="flex items-start gap-3 text-white/70">
              <svg className="w-5 h-5 mt-0.5 shrink-0 text-white/40" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
              <span className="text-[13px] sm:text-sm leading-relaxed max-w-sm">
                {settings?.address || "Shop No.1, Happy Apartment, Rajiv Gandhi Civic Centre, Near Lokendra Talkies, New Road, Ratlam (457001) M.P."}
              </span>
            </div>

            {/* Phone Numbers */}
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-6 shrink-0 text-[15px] sm:text-base text-white/90">
              {(() => {
