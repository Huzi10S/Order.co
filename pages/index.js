  ...
  return (
    <div className="min-h-screen bg-cloth pb-28">
      <header className="bg-navy text-white sticky top-0 z-20 border-b border-white/10">
        <div className="max-w-3xl mx-auto px-4 pt-4 pb-3 flex flex-col md:flex-row justify-between gap-5 relative">
          
          {/* Left Column: Name, Tagline, Address */}
          <div className="flex-1 max-w-lg">
            <div className="flex items-start justify-between md:block">
              <h1 className="text-xl sm:text-2xl font-extrabold tracking-tight text-white drop-shadow-sm mb-1.5">
                {settings?.shopName || "Supreme Sanitary"}
              </h1>
              {/* Mobile Shop login */}
              <a
                href="/shop"
                className="text-xs text-white/50 border border-white/20 rounded-full px-3 py-1.5 whitespace-nowrap md:hidden mt-0.5"
              >
                Shop login
              </a>
            </div>
            
            <p className="text-white/60 text-[11px] sm:text-xs leading-snug max-w-xl">
              Sanitary Pipes · Plumbing Pipes · Fittings · Chamber Covers · Garden & Suction Hose · Water Tanks & Bathroom Accessories
            </p>
            
            <div className="flex items-start gap-1.5 mt-3.5 text-white/60">
              <svg className="w-4 h-4 mt-0.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
              <span className="text-[11px] sm:text-xs leading-snug max-w-sm">{settings?.address || "Shop No.1, Happy Apartment, Rajiv Gandhi Civic Centre, Near Lokendra Talkies, New Road, Ratlam (457001) M.P."}</span>
            </div>
          </div>

          {/* Right Column: Shop Login (Desktop) + Phone Numbers */}
          <div className="flex flex-col md:items-end gap-3 shrink-0 md:pt-0.5">
            {/* Desktop Shop login */}
            <a
              href="/shop"
              className="hidden md:inline-flex text-xs text-white/50 border border-white/20 hover:text-white hover:border-white/50 transition rounded-full px-3 py-1.5 whitespace-nowrap self-end"
            >
              Shop login
            </a>

            {/* Phone Numbers Block */}
            <div className="flex flex-col gap-2.5 text-white/90 text-sm md:mt-3">
              {(() => {
                ...
                // Extract 10+ digit numbers, strip spaces
                ...
                return (
                  <>
                    <a href={`https://wa.me/${n1.replace(/\+/g, '')}`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-start md:justify-end gap-2 hover:text-white transition group">
                      ...
                      <span className="tracking-wide">{name1}: {n1}</span>
                    </a>
                    <a href={`tel:${n2}`} className="flex items-center justify-start md:justify-end gap-2 hover:text-white transition group">
                      <svg className="w-4 h-4 text-white/70 shrink-0 group-hover:text-white group-hover:scale-110 transition-all" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
                      <span className="tracking-wide">{name2}: {n2}</span>
                    </a>
                  </>
                );
              })()}
